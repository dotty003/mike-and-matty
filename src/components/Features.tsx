  ...
  return (
    <>
          ...
          <Reveal delay={0.1}>
            <div className="relative">
              ...
              <h2 className="text-3xl md:text-4xl mb-8 relative z-10">
                {content.forTitle} <span className="text-brand-primary italic font-serif bg-[#FFD747] px-2 text-brand-bg">{content.forAccent}</span>
              </h2>
                ...
                {content.forItems.map((item, i) => (
                  <div key={i} className="flex items-start gap-4 group">
                    <div className="w-6 h-6 rounded-full bg-brand-primary/20 flex items-center justify-center shrink-0 mt-1 group-hover:bg-brand-primary group-hover:text-white transition-colors">
                      <Check className="w-3.5 h-3.5 text-brand-primary group-hover:text-[#FFD747]" />
                    </div>
      ...
      <Section className="text-center">
        <Reveal>
          <div className="mb-20">
              <h2 className="text-4xl md:text-6xl mb-6">{content.systemTitle} <span className="text-[#FFD747]">{content.systemAccent}</span> System</h2>
          ...
          {content.featureCards.map((card, i) => {
            ...
            return (
              <Reveal key={i} delay={i * 0.2}>
                <div className="p-8 rounded-[20px] bg-brand-surface border border-white/5 hover:border-[#FFD747]/50 transition-all duration-300 group hover:-translate-y-1 h-full">
                  <div className="mb-6 w-16 h-16 rounded-[14px] bg-[#FFD747] flex items-center justify-center shadow-[0_0_20px_rgba(255,215,71,0.2)] group-hover:shadow-[0_0_40px_rgba(255,215,71,0.6)] group-hover:scale-110 transition-all duration-300 ease-out">
                    <Icon className="w-8 h-8 text-brand-bg transition-all duration-500 group-hover:scale-110" />
                  </div>
                  <h3 className="text-2xl mb-3 text-white group-hover:text-[#FFD747] transition-colors duration-300">{card.title}</h3>
